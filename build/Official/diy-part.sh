...
# 后台IP设置
export Ipv4_ipaddr="192.168.10.12"            # 修改openwrt后台地址(填0为关闭)
...
export Rootfs_partition_size="764"            # 系统分区大小,每个机型默认值不一样 (填写您想要的数值,默认一般300左右,数值以MB计算，填0为不作修改),如果你不懂就填0

# 默认主题设置
export Mandatory_theme="alpha"              # 将bootstrap替换您需要的主题为必选主题(可自行更改您要的,源码要带此主题就行,填写名称也要写对) (填写主题名称,填0为不作修改)
export Default_theme="alpha"                # 多主题时,选择某主题为默认第一主题 (填写主题名称,填0为不作修改)
...
export Customized_Information="by hza1128 $(TZ=UTC-8 date "+%Y.%m.%d")"  # 个性签名,你想写啥就写啥，(填0为不作修改)
...
git clone https://github.com/zijieKwok/luci-theme-alpha.git package/luci-theme-alpha
